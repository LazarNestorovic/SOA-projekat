...
async function likeBlog(req, res) {
	...
	try {
		...
		await pool.query('INSERT INTO likes (blog_id, user_id) VALUES ($1, $2)', [
			blog_id,
			user_id,
		]);
		...
		return res
			.status(201)
			.json({ likes_count: parseInt(countResult.rows[0].count) });
	} catch (err) {
		if (err.code === '23505') {
			return res.status(409).json({ error: 'Vec ste lajkovali ovaj blog' });
		}
		console.error(err);
		return res.status(500).json({ error: 'Greska pri lajkovanju bloga' });
	}
}
...
async function unlikeBlog(req, res) {
	...
	try {
		...
		console.error(err);
		return res.status(500).json({ error: 'Greska pri uklanjanju lajka' });
	}
}
