...
async function createComment(req, res) {
  ...
  try {
    ...
    console.error(err);
    return res.status(500).json({ error: 'Greška pri kreiranju komentara' });
  }
}
...
async function getComments(req, res) {
  ...
  try {
    ...
    console.error(err);
    return res.status(500).json({ error: 'Greška pri dohvatanju komentara' });
  }
}
...
async function updateComment(req, res) {
  ...
  try {
    ...
    if (commentCheck.rows[0].user_id !== user_id) {
      return res.status(403).json({ error: 'Nemate pravo da mijenjate ovaj komentar' });
    }
    ...
    console.error(err);
    return res.status(500).json({ error: 'Greška pri izmjeni komentara' });
  }
}
