...
function TourCard({ tour, token, onCartUpdate, onNotice, onError }) {
  ...
  return (
    <div className="card" style={{ display: 'grid', gap: '10px' }}>
      ...
      {tour.starting_point && (
        <div style={{ fontSize: '13px', color: '#66768e', display: 'flex', alignItems: 'center', gap: 6 }}>
          <span>📍</span>
          <span>Polazna tačka: <strong style={{ color: '#243551' }}>{tour.starting_point.name}</strong></span>
        </div>
      )}
      ...
      {Number(tour.review_count) > 0 && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <span style={{ fontSize: '12px', color: '#66768e' }}>({tour.review_count} recenzija)</span>
        </div>
      )}
      ...
      {tour.tags && tour.tags.length > 0 && (
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
          {tour.tags.map((tag) => (
            <span key={tag} style={{
              fontSize: '11px', background: '#f0f7ff', color: '#2e5f9e',
              borderRadius: 999, padding: '3px 9px', border: '1px solid #c8ddf5',
            }}>{tag}</span>
          ))}
        </div>
      )}
      ...
      <div style={{ marginTop: 4 }}>
        {tour.is_purchased ? (
          ...
          <button onClick={handleAdd} disabled={adding} style={{ minWidth: 130 }}>
  ...
  return (
    <div className="card panel">
      <div className="panel-head">
        <h2 style={{ fontFamily: 'Playfair Display, serif', margin: 0 }}>Objavljene ture</h2>
      ...
      {loading && tours.length === 0 ? (
        <p className="meta" style={{ textAlign: 'center', padding: '32px 0' }}>Učitavam ture...</p>
      ) : tours.length === 0 ? (
        <p className="meta" style={{ textAlign: 'center', padding: '32px 0' }}>Nema objavljenih tura.</p>
      ) : (
        <div style={{ display: 'grid', gap: 14 }}>
          {tours.map((tour) => (
            <TourCard
              ...
              tour={tour}
              token={token}
              onCartUpdate={() => { onCartUpdate(); load(); }}
              onNotice={onNotice}
              onError={onError}
            />
          ))}
