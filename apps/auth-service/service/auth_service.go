...
func (s *AuthService) Register(req *model.RegistrationRequest) (*model.User, error) {
	// Validacija ulaza
...
func (s *AuthService) Login(email, password string) (*model.AuthResponse, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("greška pri generisanju tokена: %w", err)
	}
...
// GetUserByID - pronađi korisnika po ID-u
